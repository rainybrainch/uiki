...
### deadline（期限）パラメータ ── 任意

タスクに期限がある場合（例: 「5/10 母上に報告」「6/10 納品」）、URLに `&deadline=YYYY-MM-DD` を追加：

```
https://rainybrainch.github.io/uiki/?add=<text>&project=<id>&device=<dev>&deadline=2026-05-15
```

- フォーマットは `YYYY-MM-DD` 厳守（4桁年-2桁月-2桁日）
- 不正な値は無視され、期限なしになる
- 期限なしの場合は `&deadline=` を**書かない**（空文字も書かない）
- UIKI 側に `〆 MM/DD` の刻印として静かに表示される。**催促通知・カウントダウン・赤字ハイライトは一切なし**（核1空白の彫刻維持）
